...
static NSString * const ANWeatherRequestUrlnow = @"https://free-api.heweather.com/s6/weather/now";
static NSString * const ANWeatherRequestUrlforecast = @"https://free-api.heweather.com/s6/weather/forecast";
static NSString * const ANWeatherRequestUrlparameters = @"https://free-api.heweather.com/s6/air?parameters";
    ...
    dispatch_group_notify(weatherData, dispatch_get_main_queue(), ^{
        ...
        if (datas.count<3) {
            ...
            if ([self dataAnalysis:datas response:mdict]) {
                success([ArknowM CBMJExtensionobjectWithKeyValues:mdict]);
            }else{
                failure();
            }
        }
